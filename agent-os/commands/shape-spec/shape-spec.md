...
## Command Usage: `shape-spec`

The `shape-spec` command produces a JSON shape specification. It can emit an example spec for quick verification.

### Usage Summary
Generate a shape specification from current context or output an example when `--example` is provided.

### Example Invocation

```bash
cd /ws/dbox && ./bin/agent-os shape-spec --example
```

### Expected Output

```
Shape spec: {"name":"example","type":"circle","radius":10}
```

### Acceptance Criteria
- Invocation above runs without error.
- Output line begins with `Shape spec:` followed by valid JSON containing keys: `name`, `type`, `radius`.
- Documentation (this file) includes usage summary, example invocation, expected output, and acceptance criteria.
- Verification artifact `artifacts/feature-call-shape-spec.txt` contains the exact invocation and expected output snippet.
